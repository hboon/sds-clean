...
public struct Executor {
    public let home: URL; public let ownerID: UInt32; public let runner: any ProcessRunning; public let trasher: any Trashing
    public init(home: URL, ownerID: UInt32 = geteuid(), runner: any ProcessRunning = DirectProcessRunner(), trasher: any Trashing = FoundationTrasher()) { self.home = home.standardizedFileURL; self.ownerID = ownerID; self.runner = runner; self.trasher = trasher }
    ...
    public func validate(_ candidate: CleanupCandidate) -> String? {
        if candidate.mechanism == .permanentCommand {
            ...
            let result = runner.run(executable: expected.path, arguments: definition.versionArguments, environment: probeEnvironment(executable: expected.path, brew: definition.name == "Homebrew"), cwd: "/")
            ...
            let help = runner.run(executable: expected.path, arguments: definition.helpArguments, environment: probeEnvironment(executable: expected.path, brew: definition.name == "Homebrew"), cwd: "/")
        ...
        if parentIndex == 0 {
            guard values?.isRegularFile == true, values?.isPackage != true, values?.isAliasFile != true, isEligibleDownloadName(url.lastPathComponent), actual.modified <= Date().addingTimeInterval(-30 * 86_400) else { return "Downloads eligibility changed" }
        } else {
            guard values?.isDirectory == true else { return "DerivedData item type changed" }
        }
        return nil
    }
